...
for i = 1:n_rand
    ...
    tcorner = corner(T,intmax);
    
    [xlim,ylim] = outputLimits(tform, [0 size(I,2)], [0 size(I,1)]);
    tcorner = tcorner+repmat([xlim(1),ylim(1)],size(tcorner,1),1);
    
    points = round(transformPointsForward(iform,tcorner));
    ...
    bin = zeros(size(I));
    bin(sub2ind(size(bin),p(:,1),p(:,2))) = 1;
